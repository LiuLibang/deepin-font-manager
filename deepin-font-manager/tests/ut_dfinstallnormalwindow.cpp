...
TEST_F(TestDFInstallNormalWindow, checkBatchInstall)
{
    ...
    QSignalSpy spy(iw->m_signalManager, SIGNAL(sendInstallMessage(const QStringList &)));
    iw->m_newInstallFiles.clear();
    iw->batchInstall();
...
TEST_F(TestDFInstallNormalWindow, checkOnCancelInstall)
{
    ...
    QSignalSpy spy(DFontManager::instance(), SIGNAL(cacheFinish()));
...
{
    QSignalSpy spy(iw, SIGNAL(batchReinstall(const QStringList &)));
...
{
    QSignalSpy spy(iw->m_signalManager, SIGNAL(sendReInstallMessage(const QStringList &)));
